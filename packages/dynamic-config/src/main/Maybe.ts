...
export abstract class Maybe<T> {
  ...
  public abstract ap<A, B>(this: Maybe<(val: A) => B>, maybe: Maybe<A>): Maybe<B>
...
export class Just<T> extends Maybe<T> {
   ...
   * @returns {Maybe}
   */
  public ap<A, B>(this: Maybe<(val: A) => B>, maybe: Maybe<A>): Maybe<B> {
    const self: Just<(val: A) => B> = this
    if (maybe.isJust()) {
      return new Just<B>(self.value((maybe as Just<A>).value))
    } else {
...
export class Nothing<T> extends Maybe<T> {
  ...
  public ap<A, B>(this: Maybe<(val: A) => B>, _: Maybe<A>): Maybe<B> {
    return new Nothing<B>()
  }
