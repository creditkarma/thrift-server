...
export async function createServer(
    ...
    const impl = new Calculator.Processor<Hapi.Request>({
        ...
        getUnion(index: number): SharedUnion {
            if (index === 1) {
                return SharedUnionCodec.create({ option1: 'foo' })
            } else {
                return SharedUnionCodec.create({ option2: 'bar' })
            }
        },
        ...
        checkName(choice: Choice): string {
            switch (choice.__type) {
                case ChoiceType.ChoiceWithFirstName:
                    return `FirstName: ${choice.firstName.name}`
                case ChoiceType.ChoiceWithLastName:
                    return `LastName: ${choice.lastName.name}`
                default:
                    const msg: never = choice
                    throw new Error(`Unknown choice: ${msg}`)
            }
        },
