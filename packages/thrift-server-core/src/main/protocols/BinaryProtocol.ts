...
export class BinaryProtocol extends TProtocol {
    ...
    public writeMessageBegin(name: string, type: MessageType, requestId: number): void {
        ...
        if (this.requestId) {
            logger(['warn', 'BinaryProtocol', 'writeMessageBegin'], `RequestId already set for ${name}`)
        } else {
    ...
    public writeMessageEnd(): void {
        if (this.requestId !== null) {
            ...
            logger(['warn', 'BinaryProtocol', 'writeMessageEnd'], 'No requestId to unset')
        }
    }
    ...
    public readMessageBegin(): IThriftMessage {
        ...
        if (size < 0) {
            ...
            if (version !== VERSION_1) {
                logger(['error', 'BinaryProtocol', 'readMessageBegin'], `BAD: ${version}`)
