...
export class CompactProtocol extends TProtocol {
    ...
    public writeMessageBegin(name: string, type: MessageType, requestId: number): void {
        ...
        if (this.requestId) {
          logger.warn('SeqId already set', { 'name': name })
    ...
    public readListBegin(): IThriftList {
        var sizeType: number = this.transport.readByte();
        ...
        var elementType = this.getTType(sizeType & 0x0000000f);
    ...
    public skip(type: TType): void {
        switch (type) {
            ...
            case TType.MAP:
                ...
                break;
            case TType.SET:
                var setBegin: IThriftSet = this.readSetBegin();
                ...
                break;
            case TType.LIST:
                var listBegin: IThriftList = this.readListBegin();
